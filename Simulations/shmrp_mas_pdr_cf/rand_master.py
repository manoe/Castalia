#!/bin/python3

import random
import argparse
import yaml

if __name__ == '__main__':
    parser = argparse.ArgumentParser(prog='rand.py', description='Random master node ratio generator', epilog=':-(')
    parser.add_argument('-n', '--node', dest='node', type=int)
    parser.add_argument('-r', '--ratio', dest='ratio', type=int)
    parser.add_argument('-f', '--file', dest='file')
    parser.add_argument('-e', '--external', dest='external', action='store_true')
    parser.add_argument(dest='seed', type=int)

    args = parser.parse_args()

    random.seed(args.seed)

    if args.external:
        if args.file is not None:
            loader = yaml.safe_load(open(args.file, 'r'))
            e_nodes = [i['node'] for i in loader if i['role'] == 'external']
            r_nodes = [i['node'] for i in loader if i['role'] != 'external']
            random.shuffle(e_nodes)
            random.shuffle(r_nodes)
            nodes = e_nodes.extend(r_nodes)
        else:
            raise RuntimeError('file missing')
    else:
        nodes = [x for x in range(1, args.node)]
        random.shuffle(nodes)

    if args.ratio == 100:
        for i in range(args.node):
            print(i)
    else:
        for i in range(int(args.node*args.ratio/100)):
            print(str(nodes[i]))
