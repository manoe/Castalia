...
#include "wirelessChannel/underwaterChannel/UnderWaterChannel.h"

Define_Module(UnderWaterChannel);

int UnderWaterChannel::numInitStages() const
{
	return 2;
}

void UnderWaterChannel::initialize(int stage)
{
	if (stage == 0) {
 ...
 *****************************************************************************/
void UnderWaterChannel::handleMessage(cMessage * msg)
{
	switch (msg->getKind()) {
...
void UnderWaterChannel::finishSpecific()
{
...
void UnderWaterChannel::readIniFileParameters(void)
{
 ...
 */
void UnderWaterChannel::parsePathLossMap(void)
{
...
int UnderWaterChannel::parseInt(const char *c, int *dst)
{
	while (c[0] && (c[0] == ' ' || c[0] == '\t'))
		c++;
	if (!c[0] || c[0] < '0' || c[0] > '9')
...
int UnderWaterChannel::parseFloat(const char *c, float *dst)
{
	char *tmp;
	*dst = strtof(c, &tmp);
	if (c == tmp)
