...
double WildFirePhysicalProcess::f_w_a(double angle) {
    return c_w_a * copysign(1.0,cos(angle))*pow(cos(angle),2);
}

double WildFirePhysicalProcess::f_w_s(double speed) {
    return c_w_s * sigmoid( (12+c_w_l)/c_w_m * speed - 6 - c_w_l );
}

double WildFirePhysicalProcess::sigmoid(double x) {
    ...
    for(int i=0 ; i < sense_distance*2+1 ; ++i) {
        for(int j=0 ; j < sense_distance*2+1 ; ++j) {
            if(states[i][j] == CellState::BURNING) {
                ...
                auto val=pow(1/dist,sense_attn)*(1/(1- f_w_a(calculateWindFireAngle(CellPosition(i-sense_distance,j-sense_distance))*f_w_s(wf_params.w_s ))));
